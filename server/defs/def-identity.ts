...
export const GameIdentitySchema = z.object({
	...
	horseBetWins: z.object({firsts: z.number().int().min(0), seconds: z.number().int().min(0), thirds: z.number().int().min(0)}),
	horseBetBiggestWin:  z.object({payout: z.number().int().min(0), stake: z.number().int().min(0)}),
